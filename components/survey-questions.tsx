                  ...
                  <>
                    ...
                    >
                      {question.options?.map((option, index) => (
                        <div key={option} className="flex flex-col space-y-1 border rounded-md p-3">
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value={option} id={`${question.id}-${index}`} />
                            <Label htmlFor={`${question.id}-${index}`} className="font-medium">
                              {option}
                            </Label>
                          </div>
                          <p className="text-sm text-gray-500 ml-6">{getOptionDescription(question.id, option)}</p>
                        </div>
                      ))}
                    </RadioGroup>
                    {hasError && (
